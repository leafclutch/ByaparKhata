...
export default function InventoryInsightsPage() {
  ...
  const stockStatusCounts = useMemo(() => ({
    ...
    low: products.filter((p) => getStockStatus(p.quantity, p.min_stock) === "low").length,
    out: products.filter((p) => getStockStatus(p.quantity, p.min_stock) === "out").length,
  }), [products]);

  const categoryBreakdown = useMemo(() =>
    categories.filter((c) => !c.parent_id).map((cat) => {
      const catProducts = products.filter((p) => p.category_id === cat.id || p.category_id?.startsWith(cat.id));
      const totalValue = catProducts.reduce((s, p) => s + p.quantity * p.purchase_price, 0);
      return { name: cat.name, value: totalValue, color: cat.color ?? "#94a3b8" };
    })
  , [products, categories]);

  const lowStockProducts = useMemo(() =>
    products.filter((p) => getStockStatus(p.quantity, p.min_stock) !== "ok").sort((a, b) => a.quantity - b.quantity)
  , [products]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm">
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="bg-white rounded-2xl border border-slate-100 shadow-sm">
