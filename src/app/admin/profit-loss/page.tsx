...
import { DEMO_MONTHLY_DATA, DEMO_EXPENSE_BREAKDOWN, DEMO_KPIS } from "@/lib/mock-data";
import { formatINR, formatINRCompact } from "@/lib/utils";

const plData = DEMO_MONTHLY_DATA.map((m) => ({
  month: m.month,
  revenue: m.sales,
  cogs: m.purchases,
  opex: m.expenses,
  ...
  netProfit: m.profit,
  margin: Math.round((m.profit / m.sales) * 100),
}));
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold text-slate-800">P&L Statement — May 2026</h3>
            <span className="text-xs bg-brand-50 text-brand-700 px-2 py-1 rounded-lg font-medium">Current Month</span>
          </div>
          <div className="space-y-0">
            {summaryItems.map((item, i) => {
              const change = Math.round(((item.value - item.prev) / item.prev) * 100);
              ...
              return (
                <div
                  ...
                  } ${isLast ? "border-t-2 border-slate-200 mt-1 pt-4" : ""}`}
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-sm ${isLast ? "font-bold text-slate-900" : "text-slate-600"}`}>
                      {item.label}
                    </span>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className={`text-xs flex items-center gap-0.5 ${
                      (item.positive && change > 0) || (!item.positive && change < 0)
                        ? "text-emerald-600"
                        : "text-rose-600"
                    }`}>
          ...
          <div className="mt-4 pt-3 border-t border-slate-100">
            ...
            <div className="mt-2 h-2 bg-slate-100 rounded-full">
              <div className="h-2 bg-emerald-400 rounded-full" style={{ width: `${currentMonth.margin}%` }} />
            </div>
          </div>
        </motion.div>
        ...
        >
          ...
          <ExpenseDonutChart data={DEMO_EXPENSE_BREAKDOWN} />
        </motion.div>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/50">
                ...
                <th className="text-right px-5 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">COGS</th>
                <th className="text-right px-5 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Gross Profit</th>
                <th className="text-right px-5 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">OpEx</th>
                <th className="text-right px-5 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Net Profit</th>
                <th className="text-right px-5 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Margin</th>
              </tr>
            </thead>
                ...
                <tr key={row.month} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-5 py-3 text-right">
                    <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${
                      row.margin >= 25 ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"
                    }`}>
                      {row.margin}%
                    </span>
                  </td>
