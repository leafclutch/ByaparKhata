...
const salesColumns = [
  ...
  { key: "operator", header: "Operator", hideBelow: "md" as const, render: (r: Sale) => <span className="text-xs text-slate-500">{r.operator?.full_name}</span> },
...
const expenseColumns = [
  ...
  { key: "operator", header: "Recorded By", hideBelow: "md" as const, render: (r: Expense) => <span className="text-xs text-slate-500">{r.operator?.full_name}</span> },
  { key: "payment_method", header: "Payment", hideBelow: "sm" as const, render: (r: Expense) => <StatusBadge status={r.payment_method} /> },
...
const inventoryColumns = [
  ...
  { key: "value", header: "Stock Value", hideBelow: "md" as const, render: (r: Product) => <span className="text-sm font-semibold text-slate-800">{formatNPR(r.quantity * r.purchase_price)}</span> },
];
...
export default function ReportsPage() {
  ...
  // Load data only when a tab is first opened
  useEffect(() => {
    ...
    const load = async () => {
      try {
        switch (activeTab) {
          case "sales":      setSales(await getSales(cid)); break;
          case "purchases":  setPurchases(await getPurchases(cid)); break;
          case "expenses":   setExpenses(await getExpenses(cid)); break;
          case "inventory":  setProducts(await getProducts(cid)); break;
        }
      } catch (err) {
