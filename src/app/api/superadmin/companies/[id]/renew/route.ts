...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  const { data: company, error: fetchErr } = await supabase.from("companies").select("subscription_end, name").eq("id", params.id).single();
  ...
  const { data: saUser } = await supabase.auth.admin.getUserById(params.id);
