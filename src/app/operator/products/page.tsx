...
import { getProducts, createProduct, updateProduct, deleteProduct, generateSKU } from "@/lib/services/products";
import { getCategories } from "@/lib/services/categories";
import { formatNPR, getStockStatus, getExpiryStatus, cn } from "@/lib/utils";
...
export default function ProductsPage() {
  ...
  async function openAdd() {
    ...
    if (user) {
      try {
        const sku = await generateSKU(user.company_id);
        setForm(f => ({ ...f, sku }));
      } catch {
        // SKU stays empty; user can enter it manually
      }
    }
  }
  ...
  async function handleSave() {
    if (!form.name || (!autoSku && !form.sku) || !form.selling_price) {
    ...
    try {
      const payload = {
        ...
        name: form.name, 
        sku: autoSku && !editProduct ? undefined : form.sku,
      ...
      if (editProduct) {
        const updated = await updateProduct(editProduct.id, { ...payload, sku: form.sku } as any, actor);
        setProducts((prev) => prev.map((p) => p.id === editProduct.id ? updated : p));
        toast.success("Product updated.");
      } else {
        const created = await createProduct(payload as any, actor);
  ...
  async function handleDelete() {
    ...
    try {
      ...
      await deleteProduct(deleteId, actor);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="relative max-w-sm">
        ...
        <Input placeholder="Search by name, SKU, barcode, category…" value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9 h-9 text-sm" />
      </div>
              ...
              <AnimatePresence>
                {!loading && filtered.map((product, i) => (
                  <motion.tr key={product.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ delay: i * 0.03 }} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-slate-600">{formatNPR(product.purchase_price)}</td>
                    <td className="px-4 py-3 font-semibold text-slate-800">{formatNPR(product.selling_price)}</td>
                    <td className="px-4 py-3 text-slate-700">{formatNPR(product.quantity * product.purchase_price)}</td>
          ...
          <div className="grid grid-cols-2 gap-4 py-2">
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                ...
                {isAdmin && (
                  ...
                  >
                    {autoSku ? "Manual Entry" : "Auto Generate"}
                  </button>
                )}
              </div>
              <Input 
                ...
                disabled={autoSku && !editProduct}
                className={cn(autoSku && !editProduct && "bg-slate-50 opacity-60")}
              />
              {autoSku && !editProduct && <p className="text-[10px] text-slate-400">SKU is automatically generated.</p>}
            </div>
            ...
            <div className="col-span-2 space-y-1.5">
              ...
              <Select value={form.category_id} onValueChange={(v) => {
                ...
                // If auto SKU is on, we could potentially re-generate prefix-based SKU here
              }}>
